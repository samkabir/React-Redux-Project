...
function bookReducer(state = initialState, action){
      switch(action.type) {
            ...
            case 'ADD_TO_DISCOVER_LIST': {
                  const newState = { ...state, discover: [ ...state.discover, action.payload ] };
                  return newState
            }
            case 'REMOVE_FROM_DISCOVER_LIST': {
                  const newState = { ...state, discover: state.discover.filter((book) => book.id !== action.payload )};
                  return newState
            }
            default:
                  return state
